...
1. During development, run `npm run release:verify:local`. It tests the full local
   ...
   pushing to external remotes, dispatching CI, or publishing. In the private source
   checkout it also tests release/tag repair and mismatch rejection against temporary
   local bare repositories and a fake GitHub CLI. Before the expensive suite, the gate
