...
# A malformed project settings file cannot be merged safely and would prevent both
# CONDUCTOR's native hooks and Hookify from loading. Refuse before adapter-managed
# writes instead of installing a runtime that can only look healthy on disk.
if { [ "$MODE" = "full" ] || [ "$MODE" = "strict" ]; } \
  && [ -f "$TARGET_ABS/.claude/settings.json" ] \
  && ! node "$CONDUCTOR_ROOT/bin/claude-hookify.js" state "$TARGET_ABS/.claude/settings.json" >/dev/null 2>&1; then
  echo "Error: $TARGET_ABS/.claude/settings.json is invalid or enabledPlugins is not an object; refusing a partial Claude runtime install." >&2
  exit 1
fi
...
# Emit .claude/settings.json with the Hookify project dependency, permissions allowlist,
# and hook registry. Pre-approves read-only / safe Bash commands to reduce permission-prompt friction.
...
if [ -f "$SETTINGS_PATH" ]; then
  ...
  if [ "$HOOKIFY_SETTINGS_STATE" = "missing" ] || { [ "$MISSING_CORE_HOOKS" != "invalid" ] && [ "$MISSING_CORE_HOOKS" -gt 0 ]; }; then
    ...
    log "  $SETTINGS_PATH exists — core hook registry is complete; other customizations preserved"
  fi
  if [ "$HOOKIFY_SETTINGS_STATE" = "disabled" ]; then
    log "  WARNING: $SETTINGS_PATH explicitly disables Hookify; preserving that opt-out, so .claude/hookify.* rules will not fire"
  fi
elif [ "$DRY_RUN" = "true" ]; then
  log "would write $SETTINGS_PATH with Hookify dependency + permissions.allow + hooks registry"
else
  /bin/cat > "$SETTINGS_PATH" <<'SETTINGS_EOF'
{
  "$comment": "Project-level Claude Code settings, generated by CONDUCTOR Claude adapter. Hookify powers .claude/hookify.*.local.md rules. Personal settings go in settings.local.json. Permissions allowlist pre-approves read-only/safe Bash commands; mutating commands deliberately omitted.",
  "enabledPlugins": {
...
if [ -f "$TARGET_ABS/.claude/settings.json" ]; then
  echo "  Settings: .claude/settings.json (Hookify dependency + permissions allowlist + hooks registry)"
fi
echo ""
if [ "$MODE" = "full" ] || [ "$MODE" = "strict" ]; then
  echo " Hookify: declared at project scope. On a machine where it is not installed yet,"
  echo "          approve Claude Code's official-plugin prompt (or run: claude plugin install hookify@claude-plugins-official --scope project), then /reload-plugins."
fi
