...
| `stop-cache-hit-baseline-check.sh.template` | Session stop event | Non-blocking diagnostic — reads only the current hook transcript (up to 16 MiB), deduplicates usage snapshots, computes cache-read token share, and reminds below baseline. No global session scan. Missing/oversized input skips; fail-open. Disable: `CONDUCTOR_SKIP_CACHE_CHECK=1`. | Claude `.claude/hooks/stop-cache-hit-baseline-check.sh` |
...
| `output-cap.sh.template` | Claude PostToolUse / Gemini BeforeTool | Claude: shape-preserving oversized-result replacement. Gemini: bounded combined stdout+stderr shell stream. Fail-open; see ADR-051. | Claude `.claude/hooks/output-cap.sh`; Gemini `.gemini/hooks/output-cap.sh` |
...
|---|---|
| Claude | Emits the complete verified guard set plus `output-cap.sh`; substitutes placeholders and registers native events. |
| Codex | Emits the verified PreToolUse/Stop subset. Its output cap is a native `.codex/config.toml` key, not this template. |
| Gemini | Emits this template only for the verified BeforeTool shell rewrite, plus the recipe-gated trajectory hook. |
| Cursor / Copilot / Windsurf | Emit only their verified lifecycle/recipe surfaces; unsupported hook translations remain rule obligations. |
