...
## 2. Source-of-truth + generated or per-app runtime artifacts
...
### 2.1 Runtime artifact choices

A surface may import the canonical source directly, generate locale chunks, or keep an
audited local runtime copy. Choose one declared contract per surface. Do not require a
monolithic or byte-identical copy when the runtime intentionally code-splits locales;
verify structural key/value equivalence instead.

### 2.2 Sync verification

A pre-build script verifies every generated or local artifact is byte-identical **or
structurally deep-equal** to the canonical locale map, depending on the declared build
contract. It also verifies the exact N-locale set and rejects missing, extra, empty, or
wrongly shaped locale entries.
...
## 8. Locale × surface evidence

For each material flow, the multi-surface evidence matrix records the locale set,
translation source version, generated/runtime artifact digest, relevant direction and
formatting state, accessibility label coverage, test/observation, and exact snapshot.
Automated structural equality proves synchronization; it does not prove linguistic
quality, layout fitness, cultural correctness, or assistive-technology behavior.
Those claims require the project's authorized review and remain
`verification-required` when it has not occurred.

## 9. Production failure example
